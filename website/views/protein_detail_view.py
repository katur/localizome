...
def protein_detail(request, common_name):
	...
	for video in v:
		...
		1) if strain made by Miyeko, db genotype field is simply "pJon" or "pDESTMB16"
		2) if strain not made by Miyeko and on WormBase, the genotype field is empty
		3) if strain not made by Miyeko and NOT on Wormbase, full genotype listed in the genotype field
		"""
	...
	if signals:
		...
		i = 0
		for compartment in c: # for each row
			consensus_matrix.append((compartment, signals[i:(i+num_timepoints)])) # add this row's compartment and signals
			i += num_timepoints
